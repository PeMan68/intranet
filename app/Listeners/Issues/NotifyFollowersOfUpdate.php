...
class NotifyFollowersOfUpdate
{
    ...
    public function handle(UpdatedIssue $event)
    {
		...
		if (!cache($event->issue->ticketNumber)) {
            ...
            Log::channel('templog-jobs')->debug('Cache-key updated: '.$event->issue->ticketNumber.'. Expires: '.$delay_email);
			foreach ($followers as $user) {
				SendEmailToFollowersAboutUpdate::dispatch($event->issue, $user->email, $event->type)->delay($delay_email);
                Log::channel('templog-jobs')->debug('SendEmailToFollowersAboutUpdate dispatched: '. $event->issue->ticketNumber . ' to ' . $user->email . '. type: ' . $event->type .'. Delay: ' . $delay_email);
			}
            ...
            Log::channel('templog-jobs')->debug('Dispatched new job: CreateNewReminder, '. $event->issue->ticketNumber . '. typeOfReminder: null. Delay: ' . $delay);
		}
	}
}
